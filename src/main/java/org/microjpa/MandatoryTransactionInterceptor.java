...
public class MandatoryTransactionInterceptor extends AbstractTransactionalInterceptor {

    @Override
    @AroundInvoke
    public Object transactional(InvocationContext context) throws Exception {
        if (!isTransactionActive()) {
            throw new IllegalStateException("Transaction is mandatory with @Transactional(MANDATORY), but no transaction is active");
        }
